...
void j1App::FinishUpdate()
{
	...
	/*if (last_frame_ms > 33) {
		losed_miliseconds = last_frame_ms - 33;
		SDL_Delay(losed_miliseconds);
		LOG("THE TICK MILISECONDS WHERE: %i AND WE DELAYED IT FOR %f", last_frame_ms, losed_miliseconds);
	}*/
	//SDL_Delay(1000 / framerate_cap);
	
	
  SDL_Delay((1000 / framerate_cap)-last_frame_ms);
  float i = ((1000 / framerate_cap) - last_frame_ms);
  LOG("FRAMERATE CAP IS %i AND THE DELAY IS %f", framerate_cap,i);
