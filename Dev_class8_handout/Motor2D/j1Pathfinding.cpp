...
int j1PathFinding::CreatePath(const iPoint& origin, const iPoint& destination)
{
	...
	if (IsWalkable(start) == false ||
		IsWalkable(goal) == false ||
		CheckBoundaries(start)==false||
		CheckBoundaries(goal)==false)
		return -1;
	...
	open.list.add(PathNode(0, 0, start, NULL));
		...
		for (; item; item = item->next) {
			...
			if (closed.Find(item->data.pos) == NULL) {
				...
				if (open.Find(item->data.pos) == NULL) {
					...
					item->data.CalculateF(goal);
					open.list.add(item->data);
				}
				
				// If it is already in the open list, check if it is a better path (compare G)
				else {
					if (open.Find(item->data.pos)->data.g > item->data.g + 1) {
						open.Find(item->data.pos)->data.parent = item->data.parent;
						open.Find(item->data.pos)->data.CalculateF(goal);
					}
				}
